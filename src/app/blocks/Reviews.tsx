    ...
    <div className="flex">
      {[...Array(5).keys()].map((index) => (
        <StarIcon
          ...
          className={clsx(
            'h-5 w-5',
            Number(rating) > Number(index) ? 'fill-cyan-500' : 'fill-gray-300',
          )}
          fill={fill}
        />
      ))}
...
function Review({
  ...
  Review & { pageBlockId: string; index: number; color: string }) {
  ...
  return (
    ...
    >
      <blockquote className="text-gray-900">
        <StarRating rating={rating} fill={color} />
        <p className="mt-4 flex text-lg font-semibold leading-6 before:content-['“'] after:content-['”']">
          <Text
            defaultValue={title}
            propName={`reviews.${index}.title`}
...
function ReviewColumn({
  ...
  const isBuilderMode = projectMode === 'BUILDER'
  useEffect(() => {
...
function ReviewGrid({
  ...
  if (isBuilderMode)
    return (
      <List
        defaultPropValues={reviews}
        pageBlockId={pageBlockId}
        propName="reviews"
        className="mt-16 grid auto-rows-min grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3"
        renderComponent={(review, reviewIndex) => (
          <Review
...
const Reviews: Block<ReviewsProps> = ({
  title,
  description,
  pageBlockId = '',
  reviews,
